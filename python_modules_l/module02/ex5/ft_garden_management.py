class GardenManager:
	...
	@classmethod
	def	create_plant(cls, name, water_lvl, sun_lvl):
		try:
		...
		try:
			for plants in cls.Plant.garden:
				if cls.Plant.water_tank < 1:
					raise cls.WaterError(f"Can't water {plants.name}: Water tank level too low!")
		...
		try:
			for plant in cls.Plant.garden:
				if plant.water_lvl < 1:
					raise cls.WaterError(f"Water level {plant.water_lvl} is too low (min 1)")
				elif plant.water_lvl > 10:
					raise cls.WaterError(f"Water level {plant.water_lvl} is too high (max 10)")
				elif plant.sun_lvl < 1:
					raise cls.SunError(f"Sun level {plant.sun_lvl} is too low (min 1)")
				elif plant.sun_lvl > 10:
					raise cls.SunError(f"Sun level {plant.sun_lvl} is too high (max 10)")
				print(f"{plant.name}: healthy (water: {plant.water_lvl}, sun: {plant.sun_lvl})")
		except (cls.WaterError, cls.SunError) as e:
   	 		print(f"Error checking {plant.name}: {e}")
	
	@classmethod
	def error_recovery(cls):
		print("\nTesting error recovery...")
		try:
			if cls.Plant.water_tank < 1:
				cls.Plant.water_tank = 10
				raise cls.GardenError(f"Not enough water in the tank")
